...
export default function PostClient({ postId: initialPostId }: { postId: string }) {
  const searchParams = useSearchParams();
  const postId = searchParams.get('id') || initialPostId;
  ...
  useEffect(() => {
    if (post) {
      ...
      if (twitterImage && post.imageUrl) twitterImage.setAttribute('content', post.imageUrl);
    }
  }, [post, postId]);
  ...
  const handleVote = async (type: 'like' | 'dislike') => {
    if (!postId || !post || hasVoted || isVoting) return;
